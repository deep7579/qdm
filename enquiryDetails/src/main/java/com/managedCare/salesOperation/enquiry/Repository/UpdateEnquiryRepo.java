...
public interface UpdateEnquiryRepo extends JpaRepository<GetEnquiry, Integer>{
	...
	@Modifying
	@Query(value="Update tb_enquiry set enquiry_num=:enquiryNum, address = :address,title=:title,caller_Name=:callerName,code = :code,mobile_number=:mobNo, caller_id=:callerId,relationship_id=:relationshipId,tb_referral_id=:referralId,complain=:complain,med_hist=:medHistory,doc_seen=:docCheck,height=:height,weight=:weight,feeding=:feeding,bathing=:bathing,toileting=:toileting,grooming=:grooming,ambulation=:ambulation,transfer=:transfer,bed_Mobility=:bedMobility,recommendation=:recommendation,service_preferred=:servicePreferred,rehab_criteria=:rehab,status=:status,enquiry_type_id=:enquiryTypeId,lead_time=:leadTime,created_date=current_timestamp where enquiry_id= :id",nativeQuery=true)
	int updateEnquiry(String enquiryNum,String address, String title, String callerName, String code, String mobNo, int callerId,int relationshipId, int referralId, String complain, String medHistory, boolean docCheck, String height,String weight, int feeding, int bathing, int toileting, int grooming, int ambulation, int transfer,int bedMobility, int recommendation, int servicePreferred, String rehab,int enquiryTypeId, String status, String leadTime, int id);
